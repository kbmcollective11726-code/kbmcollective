// Session starting soon (~4 min target, 2–6 min window): in-app row for every event member + push when a token exists.
...
// Target ~4 min before start (window 2–6 min). Wide enough for pg_cron (every 2 min, sometimes every 5 min)
// and small clock/network jitter without alerting too early.
const WINDOW_MIN_BEFORE_LO = 2;
const WINDOW_MIN_BEFORE_HI = 6;

/** Expo HTTP API accepts a limited number of messages per request; chunk to avoid silent failures. */
...
type EvRow = {
  ...
  const evTz = await supabase
    .from("events")
    .select("id, reminder_timezone, notifications_paused, notifications_paused_until")
    .eq("is_active", true)
  ...
  for (const e of eventRows) {
    const untilMs = e.notifications_paused_until ? Date.parse(e.notifications_paused_until) : NaN;
    const activePause =
      e.notifications_paused === true &&
      (!Number.isFinite(untilMs) || untilMs > Date.now());
    if (activePause) pausedEventIds.add(e.id);
  }
  ...
  for (const s of list) {
    if (pausedEventIds.has(s.event_id)) continue;
  ...
  if (due.length === 0) {
    return json({
      sent: 0,
      message: "No sessions starting in reminder window (timezone-aware, ~2-6 min before start)",
  ...
  let totalSent = 0;
  /** Members excluded only by same-room rule (for observability). */
  let totalSameRoomSkipped = 0;
  for (const session of toProcess) {
    ...
    const sameRoomSkippedThisSession = userIds.length - recipientUserIds.length;
    if (sameRoomSkippedThisSession > 0) {
      totalSameRoomSkipped += sameRoomSkippedThisSession;
      console.log(
        JSON.stringify({
          kind: "session_reminder_same_room_skipped",
          session_id: session.id,
          event_id: session.event_id,
          members: userIds.length,
          recipients: recipientUserIds.length,
          same_room_skipped: sameRoomSkippedThisSession,
        }),
      );
    }
    ...
    if (tokens.length > 0) {
      for (let off = 0; off < tokens.length; off += EXPO_PUSH_BATCH) {
        const batch = tokens.slice(off, off + EXPO_PUSH_BATCH);
        ...
        try {
          ...
          if (!pushRes.ok) {
            console.warn(
              "Expo push HTTP error (session reminder):",
              pushRes.status,
              rawText.slice(0, 500),
            );
          } else {
            ...
            if (okCount < batch.length) {
              console.warn(
                `Session reminder: Expo accepted ${okCount}/${batch.length} pushes (batch offset ${off}) for session ${session.id}`,
              );
            }
          }
        } catch (e) {
  ...
  return json({
    ...
    same_room_skipped: totalSameRoomSkipped,
  }, 200);
});

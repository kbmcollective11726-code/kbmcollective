-- Event admins may scan only when platform enabled session_attendance on admin_console_tiles.
...
BEGIN
  ...
  IF NOT public.is_platform_admin(auth.uid()) THEN
    SELECT e.admin_console_tiles INTO v_tiles FROM public.events e WHERE e.id = v_event_id;
    IF NOT ('session_attendance' = ANY(COALESCE(v_tiles, ARRAY[]::text[]))) THEN

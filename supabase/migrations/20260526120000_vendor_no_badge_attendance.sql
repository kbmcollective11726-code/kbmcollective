-- Vendors may save badge scan notes but cannot mark meeting / visit show-up via badge scan.
CREATE OR REPLACE FUNCTION public.upsert_badge_scan (
  p_token text,
  p_note text,
  p_attended_meeting boolean,
  p_meeting_booking_id uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_attended := coalesce(p_attended_meeting, FALSE);
  ...
  INSERT INTO public.badge_scans (
    event_id, scanner_user_id, subject_user_id, scanner_kind, attended_meeting, note, meeting_booking_id, updated_at
  ) VALUES (
    v_event_id, v_scanner, v_subject, v_kind, v_attended, NULLIF(trim(p_note), ''), v_mtg, now()
  )
  ...
  IF v_mtg IS NOT NULL AND v_kind IS DISTINCT FROM 'vendor' THEN
    ...
    VALUES (
      v_event_id, v_scanner, v_subject, v_mtg, v_attended, NULLIF(trim(p_note), ''), now()
    )

-- Per-session: hide from mobile Session check-in picker when false (web Session attendance still lists all).
ALTER TABLE public.schedule_sessions
  ADD COLUMN IF NOT EXISTS check_in_enabled BOOLEAN NOT NULL DEFAULT true;

COMMENT ON COLUMN public.schedule_sessions.check_in_enabled IS
  'When false, session is hidden from mobile Session check-in; record_session_check_in rejects new scans.';
...
BEGIN
  ...
  SELECT s.id, s.event_id, s.title, s.start_time, s.end_time, s.location, s.room, s.check_in_enabled
  INTO sess
  ...
  END IF;
  IF sess.check_in_enabled IS FALSE THEN
    RETURN jsonb_build_object('error', 'check_in_disabled');
  END IF;
